<?php
/**
 * Plugin Name: rST - Transax - WooCommerce Gateway
 * Plugin URI: https://rschooltoday.com/
 * Description: Extends WooCommerce by Adding the TRANSAX Gateway.
 * Version: 0.0.1
 * Author: API - rSchooltoday
 * Author URI: https://rschooltoday.com/
 *
...
if (!class_exists('nusoap_client')) require_once('include/lib.php');
if (!defined('ABSPATH'))
    exit;
...
function woocommerce_transax_payment_init() {
    ...
    class WC_Transax_Payment_Gateway extends WC_Payment_Gateway_CC {
        ...
        public function __construct() {
            ...
            $this->method_title = __('Credit Card', 'wctransax');
            ...
            if(!is_null($this->settings['use_refund']) && $this->settings['use_refund'] != 'no'){
              $this->supports = array(
                    'refunds'
                );
            }
			...
			add_action( 'woocommerce_order_status_refund', array($this, 'process_refund'),10,2 );
            // Actions.
...
<?php
        ...
        public function init_form_fields() {
            ...
            $this->form_fields = array(
                ...
                'rst_product' => array(
                    'title' => __('DWC Product Name', 'wctransax'),
                    'type' => 'text',
                    'description' => __('Enter DWC Product Name (OR/AS/TR/etc).', 'wctransax'),
                    'desc_tip' => true,
                    'default' => __('OR', 'wctransax')
                ),
                'instructions' => array(
			   ...
			   'use_refund' => array(
				   'title' => __('Use Refund', 'wctransax'),
				   'type' => 'checkbox',
				   'label' => __('Enable/Disable', 'wctransax'),
				   'description' => __('Allow admin to use Transax gateway on refund process', 'wctransax'),
                   'default' => 'no'
			   ),
			   'testmode' => array(
		...
		public function process_refund($order_id, $amount = null, $reason = '') {
            ...
            if ($order && $order->get_transaction_id() && !is_null($amount))
			{
                //create client
                ...
                $params['Custom_Field_1'] = !is_null($this->settings['rst_product']) ? $this->settings['rst_product'] : 'OR';
                $params['Custom_Field_2'] = get_bloginfo('name');
                $params['Custom_Field_3'] = get_bloginfo('wpurl');
                ...
                if($proxy == null) {
                    return new WP_Error('payline-error', __('Error creating proxy.', 'woocommerce'));
                }
                ...
                $wpdb->insert('dump',$body);

                if($result['ProcessTransactionResult']['STATUS_CODE']== 1){
                    update_post_meta($order_id, 'Transex Refund Transaction ID', $result['ProcessTransactionResult']['TRANS_ID']);
                    $order->add_order_note(sprintf(__('Refund %s - Refund ID: %s', 'woocommerce'), $result['ProcessTransactionResult']['STATUS_MSG'], $result['ProcessTransactionResult']['TRANS_ID']));
                    return true;
                }else{
        ...
        function process_payment($order_id) {
            //logging the last_four_digits and card types
			if(isset($_POST["last_four_digits"]) && $_POST["last_four_digits"] != "")
			{
				update_post_meta( $order_id, 'last_four_digits', $_POST["last_four_digits"] );  
			}
			else
			{
				$last_four_digits = substr($_POST['transax-card-number'], -4);
				update_post_meta( $order_id, 'last_four_digits', $last_four_digits );  
			}
			//logging the last_four_digits and card types
			if(isset($_POST["card_type"]) && $_POST["card_type"] != "")
			{
				update_post_meta( $order_id, 'card_type', $_POST["card_type"] );  
			}
			else
			{
				$ccnum = preg_replace('/(?<=\d)\s+(?=\d)/', '', trim($_POST['transax-card-number']));
				update_post_meta( $order_id, 'card_type', $this->getCardType($ccnum) );  
			}
            ...
            if($result['ProcessTransactionResult']['STATUS_CODE']== 1){
                $order->payment_complete($result['ProcessTransactionResult']['TRANS_ID']);
                update_post_meta($order_id, 'Transax Transaction ID', $result['ProcessTransactionResult']['TRANS_ID']);
                ...
                // $order->add_order_note(sprintf(__('Transaction Failed. %s-%s', 'woo-payline-rst'), $result['ProcessTransactionResult']['STATUS_CODE'], $result['ProcessTransactionResult']['STATUS_MSG']));
                wc_add_notice(sprintf(__('Transaction Failed. %s-%s', 'woo-payline-rst'), $result['ProcessTransactionResult']['STATUS_CODE'], $result['ProcessTransactionResult']['STATUS_MSG']), $notice_type = 'error');
                return;
